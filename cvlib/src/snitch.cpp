...
public:
    ...
    void setFeatureDetector(cv::Ptr<cv::Feature2D> det) { detector_ = det; }
    void setMatcher(cv::Ptr<cv::DescriptorMatcher> mat) { matcher_ = mat; }
    void setRatioThreshold(float r) { ratio_thresh_ = r; }

    /// Stitch img into pano (pano is input and output — updated pano returned)
    /// Returns true on success (homography found), false otherwise (pano unchanged)
...
private:
    ...
    static float min_element_value(const std::vector<cv::Point2f>& pts, bool x)
    {
        float v = (x ? pts[0].x : pts[0].y);
        for (size_t i = 1; i < pts.size(); ++i) v = std::min(v, (x ? pts[i].x : pts[i].y));
        return v;
    }
    static float max_element_value(const std::vector<cv::Point2f>& pts, bool x)
    {
        float v = (x ? pts[0].x : pts[0].y);
