...
//     const std::vector<int> test_indices = {0, 4, 8, 12};

//     for(int j = 4; j < gray.rows - 4; ++j) {
//         for(int i = 4; i < gray.cols - 4; ++i) {
//             uchar center = gray.ptr<uchar>(j)[i];
//             // Координаты 16 пикселей круга
//             std::vector<cv::Point2i> circle_points = {
//                 {i, j-3}, {i+1, j-3}, {i+2, j-2}, {i+3, j-1},
//                 {i+3, j}, {i+3, j+1}, {i+2, j+2}, {i+1, j+3},
//                 {i, j+3}, {i-1, j+3}, {i-2, j+2}, {i-3, j+1},
//                 {i-3, j}, {i-3, j-1}, {i-2, j-2}, {i-1, j-3}
//             };
//             // Проверяем пиксели 1, 5, 9, 13 (индексы 0, 4, 8, 12)
//             int passed_count = 0;
            
//             for (int idx : test_indices) {
//                 cv::Point2i p = circle_points[idx];
//                 uchar pixel_val = gray.ptr<uchar>(p.y)[p.x];
//                 if (std::abs(pixel_val - center) > threshold) {
//                     passed_count++;
//                 }
//             }
            
//             // Если 3 из 4 пикселей прошли проверку
//             if (passed_count >= 3) {
//                 // Проверяем все 16 пикселей
//                 bool passed[16];
//                 for (int k = 0; k < 16; ++k) {
//                     cv::Point2i p = circle_points[k];
//                     uchar pixel_val = gray.ptr<uchar>(p.y)[p.x];
//                     if (std::abs(pixel_val - center) > threshold) {
//                         passed[k] = true;
//                     }
...
//                         if (passed[idx]) {
//                             consecutive_count++;
//                             if (consecutive_count >= 9) { // N = 9 для FAST-9
//                                 is_corner = true;
//                                 break;
//                             }
//                         } else {
...
//                 if (is_corner) {
//                     keypoints.push_back(cv::KeyPoint(i, j, 7)); // 7 - размер фичи
//                 }
//             }
//         }
//     }
